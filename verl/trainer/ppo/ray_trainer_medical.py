...
from gigpo.medical_core_gigpo import compute_gigpo_outcome_advantage, compute_step_discounted_returns

from agent_system.multi_turn_rollout.medical_rollout_loop import TrajectoryCollector
from agent_system.multi_turn_rollout import adjust_batch

from agent_system.multi_turn_rollout.medical_rollout_loop import DoctorRole
...
class RayPPOTrainer:
    ...
    def _create_dataloader(self, train_dataset, val_dataset, collate_fn, train_sampler):
        ...
        print(f"Train dataset size: {len(self.train_dataset)}, Sample of train: {self.train_dataset[0]}, Validation dataset size: {len(self.val_dataset)}")
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            test_output_gen_batch = self.traj_collector.multi_turn_loop(
                                                    ...
                                                    doctor_worker_groups={DoctorRole.SPECIALIST: self.specialist_rollout_wg, DoctorRole.INTERNIST: self.internist_rollout_wg},
                                                    envs=self.val_envs,
                                                    is_train=False,
                                                    )
    ...
    def init_workers(self):
        ...
        if self.hybrid_engine:
            ...
            specialist_rollout_cls = RayClassWithInitArgs(
                cls=self.role_worker_mapping[Role.SpecialistRollout],
                config=self.config.actor_rollout_ref,
                role="specialist_rollout",
            )
            self.resource_pool_to_cls[resource_pool]["specialist_rollout"] = specialist_rollout_cls

            resource_pool = self.resource_pool_manager.get_resource_pool(Role.InternistRollout)
            internist_rollout_cls = RayClassWithInitArgs(
                cls=self.role_worker_mapping[Role.InternistRollout],
                config=self.config.actor_rollout_ref,
                role="internist_rollout",
            )
            self.resource_pool_to_cls[resource_pool]["internist_rollout"] = internist_rollout_cls
        ...
        self.internist_rollout_wg = all_wg["internist_rollout"]
        self.internist_rollout_wg.init_model()
        ...
        if self.config.actor_rollout_ref.rollout.mode == "async":
            ...
            self.async_rollout_manager = AsyncLLMServerManager(
                config=self.config.actor_rollout_ref,
                worker_group=self.specialist_rollout_wg,
            )
    ...
    def _save_checkpoint(self):
        ...
        specialist_local_path = os.path.join(local_global_step_folder, "specialist")
        internist_local_path = os.path.join(local_global_step_folder, "internist")
        ...
        self.specialist_rollout_wg.save_checkpoint(specialist_local_path, actor_remote_path, self.global_steps, max_ckpt_to_keep=max_actor_ckpt_to_keep)
        self.internist_rollout_wg.save_checkpoint(internist_local_path, actor_remote_path, self.global_steps, max_ckpt_to_keep=max_actor_ckpt_to_keep)
    ...
    def _load_checkpoint(self):
        ...
        specialist_path = os.path.join(global_step_folder, "specialist")
        internist_path = os.path.join(global_step_folder, "internist")
        critic_path = os.path.join(global_step_folder, "critic")
        # load actor
        self.specialist_rollout_wg.load_checkpoint(specialist_path, del_local_after_load=self.config.trainer.del_local_ckpt_after_load)
        self.internist_rollout_wg.load_checkpoint(internist_path, del_local_after_load=self.config.trainer.del_local_ckpt_after_load)
        # load critic
        if self.use_critic:
    ...
    def _balance_batch(self, batch: DataProto, metrics, logging_prefix="global_seqlen"):
        ...
        world_size = self.specialist_rollout_wg.world_size
                ...
                with _timer("step", timing_raw):
                    ...
                    if self.config.algorithm.adv_estimator == AdvantageEstimator.REMAX:
                        with _timer("gen_max", timing_raw):
                            ...
                            gen_baseline_output = self.specialist_rollout_wg.generate_sequences(gen_baseline_batch)
                    ...
                    with _timer("old_log_prob", timing_raw):
                        old_log_prob = self.specialist_rollout_wg.compute_log_prob(batch)
                    ...
                    if self.use_reference_policy:
                        ...
                        with _timer("ref", timing_raw):
                            if not self.ref_in_actor:
                                ...
                                ref_log_prob = self.specialist_rollout_wg.compute_ref_log_prob(batch)
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        ...
                        with _timer("update_actor", timing_raw):
                            ...
                            Internist_output = self.internist_rollout_wg.update_actor(batch)
                        specialist_output_metrics = reduce_metrics(specialist_output.meta_info["metrics"])
                        metrics.update(specialist_output_metrics)
                        internist_output_metrics = reduce_metrics(Internist_output.meta_info["metrics"])
                        metrics.update(internist_output_metrics)
